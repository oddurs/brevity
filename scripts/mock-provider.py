...
Flags:
    ...
    --fail-times N  fail the first N requests with --status, then succeed
    --retry-after S send a Retry-After header with each failure
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):  # noqa: N802  (stdlib naming)
        ...
        failing = ARGS.status >= 400 and (ARGS.fail_times == 0 or REQUESTS <= ARGS.fail_times)
        if failing:
            print(f"    -> request {REQUESTS}: replying {ARGS.status}", flush=True)
            self.respond(
                ARGS.status,
                {"error": {"message": f"mock provider says {ARGS.status}"}},
                retry_after=ARGS.retry_after,
            )
            return
        print(f"    -> request {REQUESTS}: replying 200", flush=True)
    ...
    def respond(self, status, payload, retry_after=None):
...
def main():
    ...
    parser.add_argument("--fail-times", type=int, default=0,
                        help="fail this many requests, then start succeeding")
    parser.add_argument("--retry-after", type=float, default=0.0)
